...
import { getApiStatus, getBotStatus, getExitChecks, getOpenPositions } from '@/lib/api';
...
export default function Alerts() {
  ...
  const { data: exitData } = useQuery({ queryKey: ['exit-checks'], queryFn: () => getExitChecks().then(r => r.data), refetchInterval: 10000 });
  const exits = exitData?.checks || [];
  const alerts = [
    ...
    ...exits.filter((exit: any) => exit.should_exit).map((exit: any) => ({ ok: false, title: `${exit.symbol} exit condition`, detail: `${exit.reason.replace('_', ' ')} at ₹${exit.current_price}.` })),
  ];
