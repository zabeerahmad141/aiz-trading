...
function Item({ symbol, companyName, ltp, change_pct }: { symbol: string; companyName?: string; ltp: number; change_pct: number }) {
  ...
  return (
    <span className="inline-flex items-center gap-2 px-2 text-[11px] font-mono whitespace-nowrap">
      ...
      <span className="text-text-secondary font-semibold" title={companyName}>{companyName || symbol}</span>
      <span className="text-text-muted text-[9px]">({symbol})</span>
...
export default function TickerTape() {
  const { data: quotes, isError } = useQuery({
    queryKey: ['quotes'],
  ...
  const tickerStatus = items.length ? 'Live market feed' : isError ? 'Market feed unavailable' : 'Checking market feed';
    ...
    <div className="fixed top-0 left-0 right-0 h-[34px] bg-[rgba(6,11,20,0.97)] border-b border-[var(--border)] z-[200] overflow-hidden flex items-center">
      <div className={items.length ? 'flex gap-10 animate-[ticker_50s_linear_infinite]' : 'flex items-center justify-center w-full'}>
        {items.length ? doubled.map((t: any, i: number) => (
          <Item key={i} symbol={t.symbol || t.n} companyName={t.company_name} ltp={t.ltp || t.close || 0} change_pct={t.change_pct || 0} />
