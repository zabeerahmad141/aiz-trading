...
class SignalProcessor:
    ...
    def process(self, signal: dict, candles: pd.DataFrame, capital: float) -> dict:
        ...
        levels = self.support_resistance.calculate(candles)
        ...
        if quantity <= 0:
            ...
            return result
        result.update({"approved": True, "quantity": quantity, "ltp": entry, "stop_loss": round(stop, 2), "target": round(target, 2), "atr": round(atr, 4), "regime": regime, "entry_score": entry_result["score"], "risk_reward": risk["risk_reward"], "support": levels["support"], "resistance": levels["resistance"], "reason": "Strategy and risk checks passed"})
        return result
